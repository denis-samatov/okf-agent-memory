...
| **Lifecycle** | § 5.3 | `status: draft\|stable\|deprecated`, `stale_after` using the RFC3339 profile of ISO 8601 with an explicit UTC offset. | Recommends `status` and `stale_after` without premature deletion of old context (§ 14). | **Full Match** |
